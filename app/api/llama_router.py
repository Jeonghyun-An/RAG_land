...
- 제공된 컨텍스트에 근거하여 충분히 상세하게 설명합니다(권장 8~15문장).
- 컨텍스트에 등장한 개념, 용어, 절차, 보고 흐름, 상호 관계가 있다면 풀어서 설명하세요.
- 컨텍스트에 근거가 부족하면 억지로 분량을 늘리지 말고, 실제로 설명할 수 있는 만큼만 작성하세요.
- **이미 작성한 문장이나 문단을 동일하거나 거의 동일한 표현으로 반복하지 마세요.** 더 설명할 새로운 내용이 없으면 그 지점에서 다음 섹션으로 넘어가세요.
...
- 규제적·국제적 배경을 가능한 한 상세히 설명합니다(권장 4~8문장).
- 국제 Safeguards 체제, 조약 이행, 보고 일관성 등의 맥락을 제시합니다.
- 컨텍스트에 등장하는 내용만 언급하고 추측하지 않습니다.
- 컨텍스트에 근거가 부족하면 억지로 분량을 늘리지 말고, 2)번 섹션과 같은 문장을 반복하지 마세요.

### 4) 결론(Conclusion)
- 핵심 요점을 간결하게 정리합니다(권장 2~4문장).
- 앞에서 이미 작성한 문장을 그대로 반복하지 말고, 요약하는 새로운 표현으로 정리하세요.
